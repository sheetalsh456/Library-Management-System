...
$fine = mysql_query("SELECT * FROM (SELECT Account_ID,SUM(Fine) AS Fine FROM Copy GROUP BY Account_ID) AS Copy");


$date4=date('y-m-d');
$datet=date_create($date4);
$yt = date('Y',$datet);
$mt = date('m',$datet);
$dt = date('d',$datet);

$f1 = mysql_query("SELECT * FROM Copy");

if(isset($_POST['allFine']))
{
	while($f = mysql_fetch_array($f1))
	{
		...
		$mdue = date('m',$dd);
		$ddue = date('d',$dd);
		$diff = date_diff($datet,$dd);
		
		$z = 0;
		if(1==1)
		{
			mysql_query("UPDATE Copy SET Fine = '$diff' WHERE Copy_ID='$id'");	
		}
		else
		{
			mysql_query("UPDATE Copy SET Fine = '$z' WHERE Copy_ID='$id'");	
		}
	}	
}
?>
...
<div class="col-md-4">
	<form method="post"><button type="submit" value="Search" class="submit" style="height:30px;width:150px;" name = "allFine"><center>Calculate All Fines</center></button></form>
</div>
</div>-->
<hr>
...
<div class="col-md-3">Total Fine</div>
<div class="col-md-5"><?=$due?></div>
</div>
<hr>

<?php
	 $i = 1; 
while($fine1 = mysql_fetch_array($fine))
{
	
	$sum2 = $fine1['Fine'];
  	$account_id = $fine1['Account_ID'];
  	$name = mysql_query("SELECT * FROM User WHERE Account_ID = $account_id");
  ...
  <div class="col-md-3"><?=$requested_by?></div>
  <div class="col-md-3" id="fine" onClick="goo();"><?=$sum2?></div>
  <a href="changefine.php" type="submit" style="width:40px;color:white" name="issue" id=" <?=$account_id?>" onClick="change(this.id);">Change Fine</a>
  </div>
  <hr>
...
<script type="text/javascript">
  ...
  function goo(){
    	...
    	console.log("Hi");
    	//var data = $('#fine').html();
    var editBox = '<form method="post"><div class="row"><div class="col-md-2"><input name="finetext"></div><div class="col-md-1"><button type="submit" name="changefine"></div></form>';
     //editBox.val(data);
	$(this).editBox;
    }
    </script>
