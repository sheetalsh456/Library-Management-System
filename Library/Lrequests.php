...
$req = mysql_query("SELECT * FROM Requests");
...
<?php
  	...
  	$i = 1; 
	while($row11 = mysql_fetch_array($req))
	{
  		$book_id= $row11['Book_ID'];
  		...
  		$bookname = mysql_fetch_array($book_name1);
  		$book_name = $bookname['Book_Name'];
  		$book_author1 = mysql_query("SELECT * FROM Books WHERE Book_ID = $book_id");
  		$bookauthor = mysql_fetch_array($book_author1);
  		$book_author = $bookauthor['Author_Name'];
  //$Bid = $row11['Book_ID'];
 // $_SESSION['bid'] = $row11['Book_ID'];
  ...
  <div class="col-md-2">' .$requested_by. '</div>
  <div class="col-md-3">' .$book_name. '</div>
  <div class="col-md-3">' .$book_author. '</div>
  <div class="col-md-3"><a href="onIssue.php" type="submit" style="width:40px;color:white" name="issue" id="' .$unid. '" onClick="issue(this.id);">Issue</a></div>
  </div>
  <hr>';?>
