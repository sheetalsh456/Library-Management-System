...
$copy1 = mysql_query("SELECT * FROM Copy WHERE  Account_ID = '$r'");
$copy = mysql_fetch_array($copy1);
$c = $copy['Fine'];
$co = $copy['Copy_ID'];


$req = mysql_query("SELECT * FROM User WHERE Account_ID='$r'");
...
{
	$val = mysql_real_escape_string($_POST['finee']);
	mysql_query("UPDATE Copy SET Fine='$val' WHERE Copy_ID ='$co' ");
...
if(isset($_POST['confirm']))
{
	$num_issued = mysql_query("SELECT * FROM Books WHERE Book_ID = '$bookid'");
  	...
  	if(mysql_query("UPDATE Books SET Number_Issued = '$num1' WHERE Book_ID ='$bookid'") and mysql_query("UPDATE Books SET Number_Requested='$num11' WHERE Book_ID ='$bookid'") and mysql_query("UPDATE Books SET Frequency='$f11'") and mysql_query("INSERT INTO Copy(Book_ID,Account_ID,Fine) VALUES('$bookid','$reqid','$f')") and mysql_query("DELETE FROM Requests WHERE (Book_ID='$bookid' AND Account_ID='$reqid')") )
  	{
  		?><script>alert("Issue successful");</script><?php
  		header("Location:Lrequests.php");
  	}
  	else
  	{
  		?><script>alert("Error in Issuing! Try again.");</script><?php
  	}
...
<form method="post">
<div style="font-size:20px;color:white"><b><center> User Name : <?=$requested_by?></center></b></div><br><br>
<div class="row" style="font-size:20px;color:white"><center>Fine : <input type="text"  style="color:black;" name="finee"  ></center></div>
<br><br>
<br>
<br>
<div class="row">
<center><button type="submit" value="Availabitiy" class="submit" style="height:50px;width:130px;" name = "change"><center> Change Fine<center></button></center></form>
</div>	
</div>
