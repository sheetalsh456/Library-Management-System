...
$row1 = mysql_query("SELECT * FROM Books WHERE Department='Physics'");
$reco = mysql_query("SELECT * FROM Recommendations");
...
<!--<td><form method="post"><button type="submit" style="width:10%" name="refresh">Refresh</button></form></td>-->
</div>
<hr>';?>

<?php
  ?><script>console.log("Hol");</script><?php
    $i = 1; 
  while($row11 = mysql_fetch_array($reco))
  {
      $account_id = $row11['Account_ID'];
      $book_name = $row11['R_Book_Name'];
      $book_author = $row11['R_Book_Author'];
      ...
      /*$first_name = mysql_query("SELECT First_Name FROM User WHERE Account_ID = $account_id");
      $middle_name = mysql_query("SELECT Middle_Name FROM User WHERE Account_ID = $account_id");
      $last_name = mysql_query("SELECT Last_Name FROM User WHERE Account_ID = $account_id");*/
      $recommended_by = $first_name . ' ' . $middle_name . ' ' . $last_name;
  ?>
  <?php echo'<div class="row" style="color:white">
  <div class="col-md-1">'.$i.'</div>
  <div class="col-md-3">'.$book_name.'</div>
  <div class="col-md-3">'.$book_author.'</div>
 <div class="col-md-3">'.$recommended_by.'</div>
  </div>
  <hr>';?>
