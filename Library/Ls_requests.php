...
if(isset($_POST['searchi']))
{
	   ...
	   $requested = mysql_query("SELECT * FROM Requests WHERE Account_ID = '$account_id'");
...
<tr>
<td><form method="post"><input type="text" style="width:80%;height:40px;margin-top:20px;margin-left:40px;margin-bottom:20px; " name="usearch" placeholder="Search by College ID" ></td>
<td><button type="submit" value="Search" class="submit" style="height:35px;width:90px;" name = "searchi">Go</button></form></td>
</tr>
</table>
...
<div class="col-md-4">Books Requested</div>
...
<?php
  ...
  $i = 1; 
while($requestedrow = mysql_fetch_array($requested))
 {
	    $book_id = $requestedrow['Book_ID'];
	    $unid = ($book_id*1000) + $account_id;
	    $book = mysql_query("SELECT * FROM Books WHERE Book_ID = $book_id");
	  ...
	  <div class="col-md-4"><?=$full_name?></div>
	  <div class="col-md-4"><?=$book_name?></div>
	  <div class="col-md-3"><a href="onIssue_s.php" type="submit" style="width:40px;color:white" name="issue" id=<?=$unid ?> onClick="issue(this.id);">Issue</a></div>
	  </div>
	  <hr>
	  <?php
	  ++$i;  
...
<script type="text/javascript">
      function issue(vid)
      {
      		var id = vid;
      		console.log(id);
        	$.post('sessionvarissue_s.php', {'fieldname': 'iids' , 'value2' : id});
      }
    </script>
